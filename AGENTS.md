...
- **Footer macro là NGUỒN DUY NHẤT cho 4 mục cuối bài — cấm hardcode.** Bốn mục sau BẮT BUỘC do macro `layouts/partials/post-footer.html` sinh ra từ front matter, **áp dụng cho MỌI bài dù thuộc Category nào**:
  ...
  - **`scripts/enhan_links.py`** là công cụ chính để đảm bảo mọi bài đều có cả `[[internal_links]]` FM lẫn body links. Workflow: `build_internal_link_graph.py --write` → `enhan_links.py insert` → `strip_unused_internal_links.py --write`. Script tự động thêm block `📌 Có thể bạn chưa đọc` + danh sách markdown link vào body sau đoạn giới thiệu, trước heading đầu tiên. **Không chạy `enhan_links.py` nếu bài chưa có TOML front matter + date hợp lệ.**
