...
Usage:
  ...
  python3 scripts/ml/check_pre_deploy_risk.py --all --no-healer
"""
...
def _load_model_and_meta():
    ...
    return model, meta, valid_labels


def predict_batch(filepaths: List[Path], threshold: float = 0.35, run_healer_scan: bool = False) -> List[Dict]:
    model, meta, valid_labels = _load_model_and_meta()
    if model is None:
        return []
    ...
    for fp in filepaths:
        ...
        if fm_text is None:
            fm_text = ""
        features = extract_features(fp, body_text, fm_text)
        records.append({"path": fp, "features": features})

    if not records:
        return []

    X = pd.DataFrame([{k: r["features"][k] for k in FEATURE_COLS} for r in records])
    ...
    for idx, rec in enumerate(records):
        ...
        for i, label in enumerate(FAILURE_TYPES):
            ...
            if hasattr(arr, 'shape') and len(arr.shape) == 2 and arr.shape[1] == 2:
                pos_proba = float(arr[idx, 1])
            else:
                pos_proba = float(arr.flat[idx]) if hasattr(arr, 'flat') else float(arr[idx])
        ...
        if run_healer_scan:
            try:
                subprocess.run(
                    ["python3", str(SCRIPTS_DIR / "deploy-failure-healer.py"), "--scan", "--post", str(rec["path"])],
        ...
        results.append({
            "slug": rec["path"].stem,
            "file": str(rec["path"].relative_to(REPO_ROOT)),
            "overall_risk": round(overall_risk, 4),
            "threshold": threshold,
            "ml_flagged": len(risks),
            "healer_issues_found": healer_issues,
            "combined_risks": risks,
            "skipped_ml_labels": [l for l in FAILURE_TYPES if l not in valid_labels],
            "model_hamming_loss": meta.get("hamming_loss") if meta else None,
            "n_samples_trained": meta.get("n_samples") if meta else None,
        })

    return results


def predict_single(post_path: Path, threshold: float = 0.35) -> Dict:
    model, meta, valid_labels = _load_model_and_meta()
    if model is None:
    ...
    features = extract_features(post_path, body_text, fm_text)
...
def main():
    ...
    parser.add_argument('--no-healer', action='store_true', help="Skip healer scan (faster for --all)")
    parser.add_argument('--threshold', type=float, default=0.35)
    parser.add_argument('--json', action='store_true', help="Output raw JSON")
    args = parser.parse_args()

    if args.all:
        posts = sorted((REPO_ROOT / "content" / "posts").rglob("*.md"))
        results = predict_batch(posts, threshold=args.threshold, run_healer_scan=not args.no_healer)
        if args.json:
            ...
            for r in flagged:
                print(f"   ⚠️  {r['slug']}: risk={r['overall_risk']:.1%}, issues={r['ml_flagged']}")
        return
    ...
    if args.post:
        ...
        if not path.is_absolute():
            path = REPO_ROOT / path
        result = predict_single(path, threshold=args.threshold)
        if args.json:
            ...
            if result.get("skipped_ml_labels"):
