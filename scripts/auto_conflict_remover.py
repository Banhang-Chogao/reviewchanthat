...
Only fixes:
1. Duplicate top-level frontmatter keys (Hugo fatal: mapping key already defined)
2. Trailing > in external_links entries
3. Does NOT rewrite description / ai_summary values
"""
...
def fix_external_links_trailing_chars(fm_text: str) -> str:
    ...
    for line in lines:
        if re.match(r"^\s{2,4}(title|url):", line):
