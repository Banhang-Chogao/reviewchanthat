(function(){'use strict';
var AH='78c72f67941a420cd4e5ee9fdabcaeaba6d72f16160915085f9802220fd83799',SK='mh_unlocked',DB='mh_db',API=(document.getElementById('mhApp')&&document.getElementById('mhApp').getAttribute('data-api'))||'',BASE=(document.body&&document.body.getAttribute('data-site-base'))||'';
...
function sD(){try{localStorage.setItem(DB,JSON.stringify(S.movies))}catch(e){console.warn('localStorage write fail:',e)}}

function lD(){try{var d=localStorage.getItem(DB);return d?JSON.parse(d):[]}catch(e){return[]}}
...
function aU(){S.movies=lD();rW();rC();rL()}
...
document.getElementById('mhMovieList').addEventListener('click',function(e){var t=e.target;if(t.dataset.edit){var m=S.movies.find(function(x){return x.id===t.dataset.edit});if(m)oS(m)}else if(t.dataset.delete){if(!confirm('Delete "'+(S.movies.find(function(x){return x.id===t.dataset.delete})||{}).title+'"?'))return;S.movies=S.movies.filter(function(x){return x.id!==t.dataset.delete});sD();aF();rL();uC()}});
